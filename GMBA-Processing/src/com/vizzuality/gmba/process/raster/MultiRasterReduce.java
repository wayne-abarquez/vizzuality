...
/**
 * Reads 3 parameters
 * - the first is the elevation
 * - the second the relief
 * - the third is the tpcode
 * 
 * Interpolation is by max value for the elevation and relief, and max value for
 * the tpcode
 * 
 ...
 * @author tim
 */
public class MultiRasterReduce {
	...
	protected static int[] assignColours(int[][] pixels) {
		...
		for (int i=0; i<tile.length; i++) {
			...
			int r = ((int)( pixels[0][i] * (double)0xFF / 7889)) << 16 ;
			int g = ((int)( pixels[1][i] * (double)0xFF / 3397)) << 8 ;
			int b = 0;
			if (pixels[2][i] > 0 ) {
