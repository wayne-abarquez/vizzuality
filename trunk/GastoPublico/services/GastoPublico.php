...
class GastoPublico {
    ...
    function getOrganismoData($id) {
        ...
        if($res['org_contratante']=="Administración General del Estado") {
            $sql="select grupo_fk as group_id,count(l.id) as num_licitaciones, sum(l.importe) as sum_importe,org_contratante,nombre_admin,'Madrid' as poblacion, 'Madrid' as provincia, web, alcalde, partido_politico,alcalde_voota_link,habitantes,escudo,foto,
            (SELECT x(the_geom) FROM organismo WHERE grupo_fk=o.grupo_fk LIMIT 1) as lon,
            (SELECT y(the_geom) FROM organismo WHERE grupo_fk=o.grupo_fk LIMIT 1) as lat
    ...
    function getLicitacionDetails($id) {
        $sql="select *,x(l.the_geom) as lon, y(l.the_geom) as lat from licitacion as l inner join organismo as o on l.organismo_fk=o.id where l.id=".$id." LIMIT 1";
        return pg_fetch_assoc(pg_query($this->conn, $sql));
    }
    ...
    function getOrganismosForMapaHome() {
        $sql="select id,nombre,x(the_geom) as lon,y(the_geom) as lat from grupo where the_geom is not null";
        return pg_fetch_all(pg_query($this->conn, $sql));
    }
