...
*/	
	...
	//searchresults
	    ...
	    $sql="select r.id,r.name,event_date,event_location,distance_text,(SELECT COUNT(id) FROM picture where on_id=r.id) as num_pictures,(SELECT COUNT(id) FROM comments where on_id=r.id) as num_comments,(select count(id) from users_run where run_fk=r.id) as num_users, p.name as province_name,r.province_fk as province_id,run_type,date_trunc('day',event_date) as event_day";
